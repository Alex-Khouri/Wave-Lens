...
@njit
def getLoopLength(data, frameRate, minLoop, maxLoop, autoLengthDetection):
	loopLength = -1
	loopVariance = -1
	autoTimeLimit = frameRate * 10
	if autoLengthDetection:
		minLoop = frameRate
		maxLoop = len(data) // 4
	else:
		minLoop = max(1, minLoop) * frameRate
		maxLoop = min(len(data) // 4, maxLoop * frameRate)
	for length in range(minLoop, maxLoop):
		if length % 500 == 0:
		...
		if autoLengthDetection and (length - loopLength) > autoTimeLimit:
...
class AnalyseAudioWorker(QObject):
	...
	def __init__(self, filePaths, param1, param2, typeIndex, outputIndex, output):
		...
		self.output = output
		self.autoLengthDetection = (typeIndex == 2)
	
	def printOut(self, text):
		self.progress.emit(text)

	def run(self):
		minLoop = 0.0 if self.param1 == "" else float(self.param1)
		maxLoop = 0.0 if self.param2 == "" else float(self.param2)
		if self.typeIndex == 0:
			tempo = minLoop
			beats = maxLoop
		...
		for f in range(len(self.filePaths)):
			...
			if loopLength == -1:
				...
				loopLength = getLoopLength(data, loopFrameRate, minLoop, maxLoop, self.autoLengthDetection)
				if loopLength == -1:
...
class AppGUI():
	def __init__(self):
		...
		self.boxLoopType.addItems(["Tempo/Beats", "Time Range", "Detect Automatically", "Continuous Sine Tone"])
		...
		self.layout.addWidget(self.textHelpInfo, 1, 4, 7, 1)
	...
	def setAnalyseAudioButtonStatus(self):
		if self.boxLoopType.currentIndex() == 2 or self.boxLoopType.currentIndex() == 3 or (self.lineLoopParam1.text() != "" and self.lineLoopParam2.text()) != "":
			self.buttonAnalyseAudio.setEnabled(True)
		else:
			self.buttonAnalyseAudio.setEnabled(False)
	...
	def boxLoopTypeChange(self):
		if self.boxLoopType.currentIndex() == 0:
			...
			self.buttonAnalyseAudio.setEnabled(False)
		elif self.boxLoopType.currentIndex() == 2 or self.boxLoopType.currentIndex() == 3:
			self.lineLoopParam1.setText("")
			self.lineLoopParam2.setText("")
			self.lineLoopParam1.setEnabled(False)
			self.lineLoopParam2.setEnabled(False)
