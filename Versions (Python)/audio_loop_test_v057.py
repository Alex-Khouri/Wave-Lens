...
def getLoopLength(data, frameRate, minLoop, maxLoop):
	...
	for length in range(max(1, minLoop) * frameRate, maxLoop * frameRate):
		if length % 1000 == 0: # Worker thread progress signal hack (due to incompabitility between pyqt and njit)
			print()
		list1 = data[0:length]
...
class AnalyseAudioWorker(QObject):
	...
	def run(self):
		...
		self.printOut(f"Finished! Check 'ALT Output' files for analysis results.")
		self.printOut("----------------------------------------")
		self.finished.emit()
# *****/MULTI-THREADING/*****
