...
class AnalyseAudioWorker(QObject):
	...
	def run(self):
		...
		if self.typeIndex == 0: # minLoop = Tempo, maxLoop = Number of Beats
		...
		for f in range(len(self.filePaths)):
			...
			self.printOut("Analysing loop variance...")
		...
		self.printOut(f"Finished! Check the 'Audio Loop Test Output' folder for results.")
		self.finished.emit()
# *****/MULTI-THREADING/*****
...
class AppGUI():
	def __init__(self):
		...
		self.boxLoopType.addItems(["Tempo/Beats", "Time Range", "Default"])
	...
	def boxLoopTypeChange(self):
		if self.boxLoopType.currentIndex() == 0:
			...
			self.lineLoopParam1.setReadOnly(False)
			self.lineLoopParam2.setReadOnly(False)
			self.lineLoopParam1.setStyleSheet("background-color: rgb(255,255,255);")
			self.lineLoopParam2.setStyleSheet("background-color: rgb(255,255,255);")
			...
			self.lineLoopParam2.setReadOnly(False)
			self.lineLoopParam1.setStyleSheet("background-color: rgb(255,255,255);")
			self.lineLoopParam2.setStyleSheet("background-color: white;")
			...
			self.lineLoopParam1.setReadOnly(True)
			self.lineLoopParam2.setReadOnly(True)
			self.lineLoopParam1.setStyleSheet("background-color: rgb(224,224,224);")
			self.lineLoopParam2.setStyleSheet("background-color: rgb(224,224,224);")
			self.labelLoopParam1.setText("Minimum Time (seconds):")
			self.labelLoopParam2.setText("Maximum Time (seconds):")
