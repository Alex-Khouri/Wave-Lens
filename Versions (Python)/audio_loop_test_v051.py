...
# ***** GUI *****
def boxLoopTypeChange(box, label1, label2, field1, field2):
...
def buttonOpenFileClick():
	pass

def buttonAnalyseAudioClick():
	pass
...
def drawGUI():
	...
	# labelOutput.setBuddy(textOutput)
	textFiles = QTextEdit()
	textFiles.setReadOnly(True)
	labelFiles = QLabel(text="Input Files:")
	# labelFiles.setBuddy(textFiles)
	...
	layout.addWidget(boxLoopType, 2, 2, 1, 2)
	layout.addWidget(labelLoopType, 2, 1, 1, 1, alignment=QtCore.Qt.AlignRight)
	layout.addWidget(lineLoopParam1, 3, 2, 1, 2)
	layout.addWidget(labelLoopParam1, 3, 1, 1, 1, alignment=QtCore.Qt.AlignRight)
	layout.addWidget(lineLoopParam2, 4, 2, 1, 2)
	layout.addWidget(labelLoopParam2, 4, 1, 1, 1, alignment=QtCore.Qt.AlignRight)
	layout.addWidget(textOutput, 6, 0, 1, 4)
	layout.addWidget(labelOutput, 5, 0, 1, 2)
	layout.addWidget(textFiles, 1, 0, 1, 4)
	...
	buttonOpenFile.clicked.connect(lambda: buttonOpenFileClick())
	buttonAnalyseAudio.clicked.connect(lambda: buttonAnalyseAudioClick())
	widget.show()
# *****/GUI/*****
