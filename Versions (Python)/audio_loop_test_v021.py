...
from numba.typed import List as nList # Convert lists before passing to njit functions if numba deprecates Python list support
...
def equalLists(list1, list2):
	varianceThreshold = 1.0 # Use this to calibrate equality check
...
def getLoopLength(data, sampleRate):
	...
	for length in range(minLoop, len(data) // 4):
		if equalLists(data[0:length], data[length:length * 2]):
			return length
	return -1
...
try:
	...
	for filename in argv[1:]:	
		...
		analyseData(data, audio.frame_rate, 0) # DEBUGGING
