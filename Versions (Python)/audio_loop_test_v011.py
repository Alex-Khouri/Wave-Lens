				...
				# for size in range(len(blockSizes)):
					...
					# for frame2 in range(blockSize, min(len(prevLoop), len(currLoop))):
						...
						# if prevLoopTotal == 0: # Safeguard for zero-division
							# prevLoopTotal += 1
							# currLoopTotal += 1
						# sigmaBlockVariance += 1.0 - abs((currLoopTotal - prevLoopTotal) / prevLoopTotal)
						# blockCount += 1
					# sigmaBlockVariances[size].append(sigmaBlockVariance / blockCount)
			# prevLoop = currLoop
			# start = end
	# averageLength = round(sum(loopLengths) / len(loopLengths))
	# lengthVariance = sum([(1.0 - abs((loopLengths[x] - loopLengths[x-1]) / loopLengths[x-1])) for x in range(1, len(loopLengths)) if loopLengths[x-1] != 0]) / len([length for length in loopLengths if length != 0]) 
