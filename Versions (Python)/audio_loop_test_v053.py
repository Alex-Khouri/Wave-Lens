...
def buttonOpenFileClick(window, fileNames, supportedFormats):
	fileNames.clear()
	dialog = QFileDialog()
	fileNames.extend(dialog.getOpenFileNames(filter=f"Audio Files ({' '.join([f'*.{x}' for x in supportedFormats])})")[0])
	window.setText("\n\n".join(fileNames))

def buttonAnalyseAudioClick(fileNames, param1, param2, typeIndex):
	pass

def drawGUI(supportedFormats):
	widget = QWidget()
	layout = QGridLayout()
	...
	buttonOpenFile.clicked.connect(lambda: buttonOpenFileClick(textFiles, fileNames, supportedFormats))
	buttonAnalyseAudio.clicked.connect(lambda: buttonAnalyseAudioClick(fileNames, lineLoopParam1.text(), lineLoopParam2.text(), boxLoopType.currentIndex()))
...
try:
	...
	widget = drawGUI(supportedFormats)
	widget.show()
	exit(app.exec_())
