...
# ***** HELPER FUNCTIONS *****
@njit
def compressData(data, ratio):
	ratio = round(ratio)
	if ratio < 2:
		return data
	else:
		return [round((data[x:x+ratio]) / ratio) for x in range(len(data) // ratio)]
...
def getLoopLength(data, sampleRate):
	minLoop = sampleRate // 2 # Use this to balance speed vs accurate detection of short loops
	for length in range(minLoop, len(data) // 4):
		if equalLists(data[0:length], data[length:length * 2], 0.7):
			return length
	return -1

@njit
def getAnomalies(data, loopLength):
	anomalies = []
	for loopStart in range(loopLength, len(data) - loopLength + 1, loopLength):
		...
		if not equalLists(prevLoop, currLoop, 0.8):
			anomalies.append(loopStart)
	return anomalies
# *****/HELPER FUNCTIONS/*****

# ***** DEBUGGING FUNCTIONS *****
def printData(data, frameRate, mode): # Used for debugging
	if mode == 0:
		return 0
	print("DEBUGGING: Printing data...")
	if mode == 1:
		...
		print("Invalid mode entered for 'printData()' function")
	raise SystemExit("Program terminated by successful execution of 'printData()' function")
	
def analyseData(data, frameRate, mode): # Used for debugging
	if mode == 0:
		return 0
	...
	# #######################################
		...
		sentinelSize = 100 # Measured in audio frames
		loopLengths = []
		blockSizes = [10, 25, 50, 75, 100, 250, 500, 750, 1000, 2500, 5000, 7500, 10000, 25000] # Measured in audio frames
	...
	# #######################################
		...
		print("Invalid mode entered for 'analyseData()' function")
	raise SystemExit("Program terminated by successful execution of 'analyseData()' function")
# *****/DEBUGGING FUNCTIONS/*****
...
try:
	...
	for arg in range(1, len(argv)):
		...
		compressionRatio = 1 # i.e. x:1
		sampleRate = audio.frame_rate // compressionRatio
		data = compressData(data, compressionRatio)
		print("Removing leading and trailing silence...")
		data = trimSilence(data)
		analyseData(data, sampleRate, 0) # DEBUGGING
		print("Calculating length of audio loop...")
		loopLength = getLoopLength(data, sampleRate)
		if loopLength == -1:
		...
		anomalies = getAnomalies(data, loopLength)
		if len(anomalies) > 0:
			...
			for a in range(len(anomalies)):
				# startHours = anomalies[a][0] // sampleRate // 60 // 60
				# startMinutes = anomalies[a][0] // sampleRate // 60 % 60
				# startSeconds = anomalies[a][0] // sampleRate % 60 % 60
				# endHours = anomalies[a][1] // sampleRate // 60 // 60
				# endMinutes = anomalies[a][1] // sampleRate // 60 % 60
				# endSeconds = anomalies[a][1] // sampleRate % 60 % 60
				startHours = anomalies[a] // sampleRate // 60 // 60
				startMinutes = anomalies[a] // sampleRate // 60 % 60
				startSeconds = anomalies[a] // sampleRate % 60 % 60
				endHours = (anomalies[a] + loopLength) // sampleRate // 60 // 60
				endMinutes = (anomalies[a] + loopLength) // sampleRate // 60 % 60
				endSeconds = (anomalies[a] + loopLength) // sampleRate % 60 % 60
