...
# ***** PROGRAM SETTINGS *****
lengthVarianceThreshold = 0.8 # Applies to detection of loop length
anomalyVarianceThreshold = 1.0 # Applies to detection of audio anomalies
...
@njit
def equalLists(list1, list2, frameRate, varianceThreshold, optimised):
	if len(list1) != len(list2) or len(list1) < 1:
		return False
	varianceSum = 0
	windowSize = min(len(list1), frameRate * 3) if optimised else len(list1)
...
def getLoopLength(data, frameRate, varianceThreshold):
	...
	for length in range(minLoop, len(data) // 4):
		if equalLists(data[0:length], data[length:length * 2], frameRate, varianceThreshold, True):
			return length
	return -1

@njit
def getAnomalies(data, frameRate, varianceThreshold, loopLength):
	...
	for loopStart in range(loopLength, len(data) - loopLength + 1, loopLength):
		...
		if not equalLists(prevLoop, currLoop, frameRate, varianceThreshold, False):
			if anomalies[-1][1] == loopStart:
...
try:
	...
	for arg in range(1, len(argv)):
		...
		anomalies = getAnomalies(data, audio.frame_rate, anomalyVarianceThreshold, loopLength)
		if len(anomalies) > 0:
