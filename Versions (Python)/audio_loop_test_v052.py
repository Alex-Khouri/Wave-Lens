...
def boxLoopTypeChange(box, label1, label2, field1, field2):
	if box.currentIndex() == 0:
		...
		field1.setStyleSheet("background-color: white;")
		...
		field1.setStyleSheet("background-color: lightgrey;")
		field2.setStyleSheet("background-color: lightgrey;")
		label1.setText("Minimum Time (seconds):")
		label2.setText("Maximum Time (seconds):")

def buttonOpenFileClick(window):
	dialog = QFileDialog()
...
def drawGUI():
	...
	lineLoopParam1.setValidator(QDoubleValidator())
	labelLoopParam1 = QLabel(text="Tempo (BPM):")
	lineLoopParam2 = QLineEdit()
	lineLoopParam2.setValidator(QDoubleValidator())
	...
	buttonOpenFile.clicked.connect(lambda: buttonOpenFileClick(textFiles))
