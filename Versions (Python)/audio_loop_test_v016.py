...
def analyseData(data, frameRate, mode): # Used for debugging
	...
	# #######################################
		...
		loops = []
		loopSize = 187521 # Measured in frames
		for frame in range(min(len(data), timeLength * frameRate)):
			if frame > 0 and frame % loopSize == 0:
				end = frame
				loops.append(data[start:end])
				start = end
		loopWaves = [[] for x in range(len(loops))]
		for l in range(len(loops)):
			...
			for frame in range(2, len(loop)):
				if loop[frame-1] < loop[frame] and loop[frame-1] <= loop[frame-2]:
					...
					start = frame
		waveLengths = [len(x) for x in loopWaves]
		print(f"Variance of incorrectly-calculated wave segments (n = {len(loopWaves)}; Loop Size = {loopSize}): {1 - (min(waveLengths) / max(waveLengths))}")
		raise SystemExit("Program terminated by successful execution of 'analyseData()' function")
	else:
