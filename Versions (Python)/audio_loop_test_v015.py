...
def equalLists(list1, list2): # Use this to calibrate equality check if desired loops aren't totally identical
	...
	for i in range(len(list1)):
		...
		if (1.0 - abs((value2 - value1) / value1)) > deltaThreshold:
			return False
	return True
...
def printData(data, frameRate, mode): # Used for debugging
	...
	if mode == 0:
		...
		print("Invalid mode entered for 'printData()' function; proceeding with execution")
...
def analyseData(data, frameRate, mode): # Used for debugging
	...
	# #######################################
		...
		raise SystemExit("Program terminated by successful execution of 'analyseData()' function")
	# #######################################
	# Variance data (signal contour broken into individual wave crests) -> EUREKA
	# #######################################
	elif mode == 4:
		...
		print(f"Variance of correctly-calculated wave lengths (n={len(loopWaves)}): {1 - (min(waveLengths) / max(waveLengths))}")
		print("Correctly-calculated wave lengths:")
		for i in range(10):
			print(waveLengths[i])
		print("...")
		...
		for l in range(len(loops)):
			...
			for frame in range(2, len(loop)):
				if loop[frame-1] < loop[frame] and loop[frame-1] <= loop[frame-2]:
					...
					start = frame
		waveLengths = [len(x) for x in loopWaves]
		print(f"Variance of incorrectly-calculated wave lengths (n={len(loopWaves)}): {1 - (min(waveLengths) / max(waveLengths))}")
		print("Incorrectly-calculated wave lengths:")
		for i in range(10):
			print(waveLengths[i])
		print("...")
		raise SystemExit("Program terminated by successful execution of 'analyseData()' function")
	else:
		print("Invalid mode entered for 'analyseData()' function; proceeding with execution")
# *****/DEBUGGING FUNCTIONS/*****
...
try:
	...
	for filename in argv[1:]:	
		...
		analyseData(data, audio.frame_rate, 4) # DEBUGGING
