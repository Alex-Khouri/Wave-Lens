...
try:
	...
	while loopLengthType == -1:
		try:
			...
			if loopLengthType == 1:
				validLengthDetails = False
				while True:
					try:
						loopBeats = float(input("Number of Beats: "))
						if loopBeats < 1.0:
							raise ValueError()
						else:
							break
				...
				while True:
					try:
						loopTempo = float(input("Tempo (BPM): "))
						if loopTempo < 1.0:
							raise ValueError()
						else:
							break
					except:
						print("INVALID INPUT - please enter a tempo/BPM >= 1")
				minLoop = floor((60.0 / loopTempo) * loopBeats) - 1
				maxLoop = ceil((60.0 / loopTempo) * loopBeats) + 1
			elif loopLengthType == 2:
				while True:
					try:
						minLoop = floor(float(input("Minimum time value (seconds): ")))
						if minLoop < 1:
							raise ValueError()
						else:
							break
				...
				while True:
					try:
						maxLoop = ceil(float(input("Maximum time value (seconds): ")))
						if maxLoop <= minLoop:
							raise ValueError()
						else:
							break
