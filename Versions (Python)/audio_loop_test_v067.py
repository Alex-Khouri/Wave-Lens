...
def drawVariances(variances, loopLength, frameRate, fileName, outputIndex):
...
class AnalyseAudioWorker(QObject):
	...
	def run(self):
		...
		for f in range(len(self.filePaths)):
			...
			drawVariances(variances, loopLength, audio.frame_rate, fileName, self.outputIndex)
...
class AppGUI():
	def __init__(self):
		...
		self.boxOutputType.addItems(["PNG", "PDF"])
