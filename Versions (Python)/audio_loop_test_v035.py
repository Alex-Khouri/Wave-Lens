...
anomalyDetectionFactor = 2.0 # Applies to detection of audio anomalies
# *****/PROGRAM SETTINGS/*****
...
try:
	...
	for arg in range(1, len(argv)):
		...
		anomalies, variances = getAnomalies(data, audio.frame_rate, loopLength, loopVariance, anomalyDetectionFactor)
		outputFilename = f"Audio Loop Test Output {arg:03}.csv"
		fileCSV = open(outputFilename, "w")
		fileCSV.write(",".join([str(x) for x in variances]))
		fileCSV.close()
		if len(anomalies) > 0:
			...
			for a in range(len(anomalies)):
				startHours = (anomalies[a][0] + startOffset) // audio.frame_rate // 60 // 60
				startMinutes = (anomalies[a][0] + startOffset) // audio.frame_rate // 60 % 60
				startSeconds = (anomalies[a][0] + startOffset) // audio.frame_rate % 60 % 60
				endHours = (anomalies[a][1] + startOffset) // audio.frame_rate // 60 // 60
				endMinutes = (anomalies[a][1] + startOffset) // audio.frame_rate // 60 % 60
				endSeconds = (anomalies[a][1] + startOffset) // audio.frame_rate % 60 % 60
