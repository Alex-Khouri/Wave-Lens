...
# ***** PROGRAM SETTINGS *****
compressionRatio = 1 # i.e. x:1; This requires recalibrating (increasing?) `lengthVarianceThreshold` (check loop length)
...
def compressData(data, ratio):
	...
	if ratio < 2:
		...
		return [round(sum(data[x:x+ratio]) / ratio) for x in range(0, len(data), ratio)]
...
try:
	...
	for arg in range(1, len(argv)):
		...
		print("Compressing audio data...")
		while audio.frame_rate % compressionRatio != 0: # Ensures time measurement accuracy and reduces data truncation
			compressionRatio -= 1
		frameRate = audio.frame_rate // compressionRatio
		data = compressData(data, compressionRatio)
		debugData(data, frameRate, 0)
		print("Calculating length of audio loop...")
		loopLength = getLoopLength(data, frameRate, lengthVarianceThreshold)
		if loopLength == -1:
		...
		if len(anomalies) > 0:
			...
			for a in range(len(anomalies)):
				startHours = anomalies[a] // frameRate // 60 // 60
				startMinutes = anomalies[a] // frameRate // 60 % 60
				startSeconds = anomalies[a] // frameRate % 60 % 60
				endHours = (anomalies[a] + loopLength) // frameRate // 60 // 60
				endMinutes = (anomalies[a] + loopLength) // frameRate // 60 % 60
				endSeconds = (anomalies[a] + loopLength) // frameRate % 60 % 60
