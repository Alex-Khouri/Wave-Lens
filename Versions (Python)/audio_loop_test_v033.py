...
detectionFactor = 1.5 # Applies to detection of audio anomalies
# *****/PROGRAM SETTINGS/*****
...
@njit
def getAnomalies(data, frameRate, loopLength, loopVariance, detectionFactor):
	anomalies = [] # Format: [[anomaly_1_start, anomaly_1_end], [anomaly_2_start, anomaly_2_end], etc]
	for loopStart in range(loopLength, len(data) - (2 * loopLength) + 1, loopLength):
		prevLoop = data[loopStart - loopLength:loopStart]
		currLoop = data[loopStart:loopStart + loopLength]
		nextLoop = data[loopStart + loopLength:loopStart + (2 * loopLength)]
		backVarianceSum = 0
		forwardVarianceSum = 0
		for i in range(len(currLoop)):
			value1 = prevLoop[i]
			value2 = currLoop[i]
			value3 = nextLoop[i]
			while value1 == 0 or value2 == 0: # Safeguard against zero-division below
				value1 += 1
				value2 += 1
				value3 += 1
			backVarianceSum += abs((value2 - value1) / value1)
			forwardVarianceSum += abs((value3 - value2) / value2)
		backVariance = backVarianceSum / len(currLoop)
		forwardVariance = forwardVarianceSum / len(currLoop)
		if backVariance > (loopVariance * detectionFactor) or forwardVariance > (loopVariance * detectionFactor):
