...
def equalLists(list1, list2):
	deltaThreshold = 0.01 # Use this to calibrate equality check
	if len(list1) != len(list2):
		return False
	variances = []
	for i in range(len(list1)):
		# Safeguard against zero-division below
		value1 = list1[i] + (1 / deltaThreshold) if list1[i] == 0 else list1[i] 
		value2 = list2[i] + (1 / deltaThreshold) if list1[i] == 0 else list2[i]
		variances.push(abs((value2 - value1) / value1))
	outliers = [x for x in variances if x > deltaThreshold]
	return len(outliers) / len(list1) <= deltaThreshold
...
def analyseData(data, frameRate, mode): # Used for debugging
	...
	# #######################################
		...
		file.close()
	# #######################################
	# Variance data (signal contour broken into individual wave crests) -> EUREKA (find minimum wave length variance)
	# #######################################
	elif mode == 4:
		...
		correctLoop = 187520 # Measured in frames
	...
	# #######################################
	# Loop data (frame variance)
	# #######################################
	elif mode == 5:
		correctLoop = 187517 # Measured in frames
		file = open("Audio Loop Test Frame-Wise Variance Data.txt", "w")
		for frame in range(correctLoop):
			value1 = data[frame]
			value2 = data[frame + correctLoop]
			if value1 == 0: # Safeguard against zero-division below
				value1 += 1
				value2 += 1
			file.write(f"Variance at frame {frame:06}: {abs((value2 - value1) / value1)}\n")
		file.close()
	else:
...
try:
	...
	for filename in argv[1:]:	
		...
		analyseData(data, audio.frame_rate, 5) # DEBUGGING
