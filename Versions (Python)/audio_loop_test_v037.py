...
def drawVariances(variances, loopLength, frameRate, outputFilename):
	...
	pyplot.title(outputFilename.split(".")[0])
	pyplot.xlabel("Time")
	...
	print(xLabels) # To Fix: Last tick label isn't being displayed
	axes.set_xticklabels(xLabels, rotation=45, horizontalalignment='right')
	
	PdfPages.savefig(outputFilename, bbox_inches='tight')
# *****/HELPER FUNCTIONS/*****
...
try:
	...
	for arg in range(1, len(argv)):
		...
		outputFilename = f"Audio Loop Test Output {arg:03}.pdf"
		drawVariances(variances, loopLength, audio.frame_rate, outputFilename) # UNFINISHED: Draw graph then export image file
