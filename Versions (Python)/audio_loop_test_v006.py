...
def printData(data, frameRate): # Used for debugging
	...
	file = open("Audio Loop Test Raw Data.csv", "w")
...
def analyseData(data, frameRate): # Used for debugging
	...
	loops = []
	sentinelSize = 100
	for frame in range(sentinelSize, len(data)):
		if data[frame] != 0 and len([x for x in data[frame-sentinelSize:frame] if x == 0]) == sentinelSize:
			end = frame
	...
	sigmaVariance = 0.0
	blockCount = 0
	for frame in range(blockSize, min(len(loops[0]), len(loops[1]))):
		loop1Total = sum(loops[0][frame-blockSize:frame])
		loop2Total = sum(loops[1][frame-blockSize:frame])
		if loop1Total == 0: # Safeguard for zero-division
			loop1Total += 1
			loop2Total += 2
		sigmaVariance += (loop2Total - loop1Total) / loop1Total
		blockCount += 1
	print(f"Average variance between {blockSize}-frame blocks: {sigmaVariance/blockCount}")
