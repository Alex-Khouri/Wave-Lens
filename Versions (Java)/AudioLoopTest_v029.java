...
class AnalyseAudio extends Task<Integer> {
	...
	public float average(ArrayList<Float> items) {
		if (items.size() == 1) { return items.get(0); }
		float total = 0.0f;
		for (float item : items) { total += item; }
	...
	public void drawVariances(int loopLength, int frameRate, ArrayList<Float> variances, String fileName, int outputIndex) {
		ArrayList<Float> smoothedVariances = new ArrayList<Float>(); 		// 10-point average
		ArrayList<Float> ultraSmoothedVariances = new ArrayList<Float>();	// 20-point average
		ArrayList<Integer> timestamps = new ArrayList<Integer>(); 			// Measured in seconds
		for (int v = 0; v < variances.size(); v++) {
			smoothedVariances.add(average(new ArrayList<Float>(variances.subList(Math.max(v - 9, 0), v + 1))));
			ultraSmoothedVariances.add(average(new ArrayList<Float>(variances.subList(Math.max(v - 19, 0), v + 1))));
			timestamps.add(Math.round(v * ((float)loopLength / frameRate)));
		}
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		for (int v = 0; v < variances.size(); v++) {
			dataset.addValue(variances.get(v), "Raw Data", timestamps.get(v));
			dataset.addValue(smoothedVariances.get(v), "10-Point Average", timestamps.get(v));
			dataset.addValue(ultraSmoothedVariances.get(v), "20-Point Average", timestamps.get(v));
		}
		JFreeChart lineChart = ChartFactory.createLineChart(
			"Audio Loop Test Output\n" + fileName,	// Chart title
			"Time",									// X-Axis Label
			"Loop Variance", 						// Y-Axis Label
			dataset									// Dataset for the Chart
			);
		try {
			if (outputIndex == 0) {
				ExportUtils.writeAsPNG(lineChart, 1200, 900, new File("[ALP Output] " + safePath(fileName) + ".png"));
			} else if (outputIndex == 1) {
				ExportUtils.writeAsPDF(lineChart, 1200, 900, new File("[ALP Output] " + safePath(fileName) + ".pdf"));
			}
		} catch (Exception e) {
