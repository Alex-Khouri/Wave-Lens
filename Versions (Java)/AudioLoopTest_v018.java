...
class AnalyseAudio extends Task<Integer> {
	...
	public ArrayList<Integer> getSampleList(byte[] buffer, int bitRate, int channels, int frameSize, boolean bigEndian) {
		...
		for (int f = start; f < buffer.length; f += frameSize) {
		...
		silence = true;
		for (int f = end; f > start; f -= frameSize) {
			for (int b = 0; b < frameSize; b++) {
				if (buffer[f + b] != 0) {
					end = f;
					silence = false;
		...
		for (int f = start; f < end; f += frameSize) {	// Frame
			int[] frame = new int[channels];
			for (int c = 0; c < channels; c++) {					// Channel
				for (int b = 0; b < channelSize; b++) {				// Byte
