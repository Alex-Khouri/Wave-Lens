	...
	public void drawVariances(int loopLength, int frameRate, ArrayList<Float> variances, String fileName, int outputIndex) {
		try {
			...
			for (int v = 0; v < variances.size(); v++) {
				currTimestamp = Math.round(v * ((float)loopLength / frameRate));
				for (int s = 0; s < (currTimestamp - prevTimestamp); s++) { current = (Second)current.next(); }
				series1.add(current, variances.get(v));
				series2.add(current, average(new ArrayList<Float>(variances.subList(Math.max(v - 9, 0), v + 1))));
				series3.add(current, average(new ArrayList<Float>(variances.subList(Math.max(v - 19, 0), v + 1))));
				prevTimestamp = currTimestamp;
			}
			TimeSeriesCollection dataset = new TimeSeriesCollection();
			dataset.addSeries(series3);	// Series are layered in the order they're added (i.e. first = top)
			...
			if (outputIndex == 0) {
				ExportUtils.writeAsPNG(chart, 1500, 900, new File("[ALP Output] " + safePath(fileName) + ".png"));
			} else if (outputIndex == 1) {
				ExportUtils.writeAsPDF(chart, 1500, 900, new File("[ALP Output] " + safePath(fileName) + ".pdf"));
			}
		} catch (Exception e) {
...
public class AudioLoopTest extends Application {
	...
	// GUI Data
	String[] supportedFormats = new String[] {"aiff", "au", "wav"};
    ...
    public void start(Stage stage) {
		...
		stage.getIcons().add(new Image("file:Images/audio_loop_test.png"));
