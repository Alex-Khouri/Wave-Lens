...
class AnalyseAudio extends Task<Integer> {
	...
	protected Integer call() throws Exception {
		...
		try {
			...
			for (File file : openFiles) {
				...
				ArrayList<Integer> variances = getLoopVariances(currentLoopLength, data);	// TBC
		...
		this.printOut("----------------------------------------");
		System.out.println("Program Execution Time: " + (((double)System.currentTimeMillis() - (double)start) / 1000) + " seconds");
		return 0;
	}
	...
	public int getLoopLength(ArrayList<Integer> data, int frameRate, int minLoop, int maxLoop, boolean autoLengthDetection) {
		...
		for (int length = minLoop; length < maxLoop; length++) {
			...
			for (int i = 0; i < list1.size(); i++) {
				float value1 = list1.get(i) == 0 ? (float)list1.get(i) + 1.0f : (float)list1.get(i);	// Guard against zero-division below
				float value2 = list1.get(i) == 0 ? (float)list2.get(i) + 1.0f : (float)list2.get(i);	// "list1.get(i) == 0" is not a typo
	...
	public ArrayList<Integer> getLoopVariances(int loopLength, ArrayList<Integer> data) {
		ArrayList<Integer> variances = new ArrayList<Integer>();
		return variances;
	}
