	...
	protected Integer call() throws Exception {
		try {
			...
			for (File file : openFiles) {
				...
				// Audio frames are stored as 32-bit integers, but are currently being read as 8-bit bytes.
				// Modify this to convert byte stream into integer array (for processing).
				AudioInputStream audioStream = AudioSystem.getAudioInputStream(file);
				byte[] buffer = new byte[(int) file.length()];
				audioStream.read(buffer);
				audioStream.close();
				float currentFrameRate = audioStream.getFormat().getSampleRate();
				printOut(currentFrameRate);
