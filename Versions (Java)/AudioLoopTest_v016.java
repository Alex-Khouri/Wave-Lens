...
class AnalyseAudio extends Task<Integer> {
	...
	protected Integer call() throws Exception {
		try {
			...
			for (File file : openFiles) {
				...
				this.printOut("Removing leading and trailing silence...");
				// TBC
			}
		} catch (Exception e) {
	...
	public ArrayList<Integer> getSampleList(byte[] buffer, int bitRate, int channels, int frameSize, boolean bigEndian) {
		...
		for (long f = 0; f < buffer.length; f += frameSize) {	// Frame
			int[] frame = new int[channels];
			for (int c = 0; c < channels; c++) {				// Channel
				int bytes = bitRate / 8;
				for (int b = 0; b < bytes; b++) {				// Byte
					byte currentByte = buffer[f + (c * bytes) + b];
					if (bigEndian) {
						frame[c] &= (currentByte << ((bytes - 1 - b) * 8));
					} else {
						frame[c] &= (currentByte << (b * 8));
					}
				}
			}
			float monoFrame = 0.0f;
			for (int channel : frame) { monoFrame += Float.valueOf(channel); }
			monoFrame /= channels;
			output.add(Math.round(monoFrame));
		}
		return output;
	}
