...
class AnalyseAudio extends Task<Integer> {
	...
	protected Integer call() throws Exception {
		...
		try {
			...
			for (File file : openFiles) {
				...
				ArrayList<Integer> variances = getLoopVariances(currentLoopLength, data);
	...
	public ArrayList<Float> getLoopVariances(int loopLength, ArrayList<Integer> data) {
		...
		for (int loopStart = loopLength; loopStart <= (data.size() - loopLength); loopStart += loopLength) {
			ArrayList<Integer> prevLoop = data.subList(loopStart - loopLength, loopStart);
			ArrayList<Integer> currLoop = data.subList(loopStart, loopStart + loopLength);
			float varianceSum = 0.0f;
			for (int i = 0; i < currLoop.size(); i++) {
				...
				float varianceSum += Math.abs((float)(value2 - value1) / value1);
			}
